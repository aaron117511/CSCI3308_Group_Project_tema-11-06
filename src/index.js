...
const auth = (req, res, next) => {
  if (req.path === '/getUserInfo' || req.path === '/getUserTopArtists' || req.path === '/getUserTopTracks' || req.path === '/createPlaylist' || req.path === '/addTracks' || req.path === '/artist' || req.path === '/track') {
    return next();
  }
...
app.get('/trackImage', async (req, res) => {
  const track_response = axios.get(url_concat + '/track?key=' + req.session.user.access_token + '&id=' + req.query.track_id);
  const album_image = track_response.data.album.images[0].url;
  res.redirect(album_image);
});

app.get('/artistImage', async (req, res) => {
  const artist_response = axios.get(url_concat + '/artist?key=' + req.session.user.access_token + '&id=' + req.query.artist_id);
  const artist_image = artist_response.data.images[0].url;
  res.redirect(artist_image);
});
...
app.get('/track', (req, res) => {
  ...
  .catch(error => {
    ...
    // If token is expired, call /refresh route to refresh the token, and then have this route call itself again.
    if (status == 401) {
      console.log("/track: Status 401 received. Refreshing token...")
      res.status(status).send('Token refresh required');
    }

    else {
      console.log("/track error: There was an error in retrieving API data. See detailed error below:");
      console.log(error.response.data);

      if (status == 400) {console.log('/track Error: Status 400 received');}
      if (status == 403) {console.log('/track Error: Bad OAuth Request');}
      else if (status == 429) {console.log('/track Error: Rate Limit Exceeded');}
      res.status(status).send('Error');
...
app.get('/artist', (req, res) => {
  ...
  .catch(error => {
    ...
    // If token is expired, call /refresh route to refresh the token, and then have this route call itself again.
    if (status == 401) {
      console.log("/artist: Status 401 received. Refreshing token...")
      res.status(status).send('Token refresh required');
    }

    else {
      console.log("/artist error: There was an error in retrieving API data. See detailed error below:");
      console.log(error.response.data);

      if (status == 400) {console.log('/artist Error: Status 400 received');}
      if (status == 403) {console.log('/artist Error: Bad OAuth Request');}
      else if (status == 429) {console.log('/artist Error: Rate Limit Exceeded');}
      res.status(status).send('Error');
